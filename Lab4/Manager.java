...
public class Manager {
    ...
    private List<ParallelExecutor> exList = new ArrayList<>();
    ...
    public ErrorsHandle ReadConfig (String configFilename) {
        ...
        if (writer.status() != Status.OK ) {
            logger.log(Level.INFO, "Problems with writing.");
        ...
        if (execNumber == 0) {
            //случай, если 0 экзекьюторов
            reader.addConsumer(writer);
            writer.addProducer(reader);
        }
        else {
            for (ParallelExecutor ex: exList) {
    ...
    public void runConvey() {
        try {
            ...
            for (Executor ex : exList) {
                Thread th = new Thread(ex);
                threads.add(th);
                th.start();
                System.out.println(th.getName() + " is started");
            }
            ...
            for (Thread th : threads) {
                while (th.isAlive())
                    th.interrupt();
                System.out.println(th.getName() + " is closed");
            }
            if (reader.status() == Status.OK) {
            ...
            logger.log(Level.INFO, "Threads exception: ", ex);
        }
    }



    private ErrorsHandle CreateExecutor (String className, String config, Logger logger, ParallelProducer producer)
    {
        ParallelExecutor execInst;
        try {
            Class c = Class.forName(className);
            Class[] params = {String.class, ru.spbstu.pipeline.logging.Logger.class};
            execInst = (ParallelExecutor) c.getConstructor(params).newInstance(config, UtilLogger.of(logger));
        }
    ...
    public static void main(String[] args) {
        ...
        System.out.println("Running convey!");
        manager.runConvey();
    }
