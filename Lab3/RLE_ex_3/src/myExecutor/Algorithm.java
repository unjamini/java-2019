...
public class Algorithm {
    ...
    static byte[] RLE (byte[] input) {
        ...
        while (i < input.length) {
            ...
            if (seqLen == 1) {
                ...
                while (i + seqLen < input.length && input[i + seqLen] != input[i + seqLen - 1] && seqLen < Byte.MAX_VALUE / 2) {
                    seqLen++;
                }
                if (input[i + seqLen - 1] == input[i + seqLen - 2])
                    seqLen--;
                byte decoder = 0;
                decoder -= seqLen;
                result.add(decoder);
    ...
    static byte[] RLE_Decode (byte[] input) {
        ...
        for (int i = 0; i < input.length; i++) {
            byte decoder = (byte) -input[i++];
            byte length = (byte) ((byte)(decoder << NUMBER_OF_SUPPL_BITS) >> NUMBER_OF_SUPPL_BITS);
            decoder >>= SUPPL_BIT_POSITION;
            if (decoder > 0) {
                //последовательность одинаковых байтов
